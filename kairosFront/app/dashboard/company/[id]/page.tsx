...
export default function CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadOutreachMessage = async (selectedCompany: Company): Promise<string> => {
    const session = readKairosAccountSession()

    if (session === null) {
      throw new Error('Unable to create outreach: expected authenticated account session.')
    }
  ...
  const loadSuggestedOutreach = async (selectedCompany: Company): Promise<void> => {
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
        <div className="space-y-6">
          <Section title="Why this is relevant">
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <ContextMetric label="Primary signal" value={recommendation.primarySignalLabel} />
                <ContextMetric label="Recommended Action" value={recommendation.actionLabel} />
                <ContextMetric label="Service fit" value={serviceRecommendation} />
                <ContextMetric label="Confidence" value={recommendation.confidenceLabel} />
              </div>
              <p className="text-sm text-muted-foreground">
                {recommendation.noPredictionNotice} It only explains why this moment may be
                more relevant for your selected service.
              </p>
            </div>
          </Section>
          ...
          <Section title="Timing analysis">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Score is service-aware and uses age, digital signals, confidence, and the selected
              service. It does not predict purchase intent.
            </p>
          </Section>
            ...
            {company.digitalSignals && company.digitalSignals.length > 0 ? (
              <div className="grid gap-3">
                {company.digitalSignals.map((signal) => (
                  <div key={signal.signalName} className="rounded-lg border border-border p-3">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      Source: {signal.sourceName}. Impact: {signal.serviceImpact}.
                    </p>
              ...
              <p className="text-sm text-muted-foreground">No digital signals recorded yet.</p>
            )}
          </Section>
                ...
                {history.map((entry) => (
                  <div key={entry.id} className="rounded-lg border border-border p-3">
                    <p className="text-sm font-medium">
                      {entry.previousStage ? getTimingStageLabel(entry.previousStage) : 'Initial'} to{' '}
                      {getTimingStageLabel(entry.nextStage)}
                    </p>
                    <p className="mt-1 text-sm text-muted-foreground">
        ...
        <aside className="space-y-4">
          <Section title="Actions">
            <div className="space-y-3">
              <Button className="w-full justify-start gap-2" onClick={handleSave}>
          ...
          <Section title="Company information">
            ...
            />
            <InfoRow label="Registered Date" value={company.registeredDate.toLocaleDateString()} />
            ...
            <InfoRow label="Registry ID" value={company.sourceDocumentNumber ?? 'Not available'} />
            <InfoRow label="Source" value={company.source} />
            <InfoRow label="Stage" value={getTimingStageLabel(company.timingStage)} />
            <InfoRow label="Signals" value={String(company.digitalSignals?.length ?? 0)} />
          </Section>
        </aside>
      </div>
    </div>
...
function SignalMetadata({
  ...
  if (!hasTechnologies && !hasSocialProfiles && !hasContactMethods && metadata.websiteUrl === undefined) {
    return null
  }
  ...
  return (
    <div className="mt-3 space-y-2">
      ...
      {hasContactMethods && <ContactMethodList contacts={metadata.contactMethods ?? []} />}
      {hasTechnologies && <TechnologyList technologies={metadata.technologies ?? []} />}
      {hasSocialProfiles && <SocialProfileList profiles={metadata.socialProfiles ?? []} />}
    </div>
  )
}
...
function resolveConfiguredOfferedService(): OfferedService {
  return readKairosMarketTargetSession()?.offeredService ?? 'website-design-development'
}
...
function OutreachMessagePanel({
  ...
  if (error !== null) {
    return <div className="mb-4 rounded-lg bg-destructive/10 p-4 text-sm text-destructive">{error}</div>
  }

  return (
    <div className="mb-4 rounded-lg bg-muted/50 p-4 font-mono text-sm whitespace-pre-wrap">
      {isLoading ? 'Generating suggested outreach...' : message ?? 'Suggested outreach is unavailable.'}
    </div>
  )
}
...
function DetailState({ message }: { message: string }) {
  return <div className="surface-card rounded-lg p-12 text-center text-muted-foreground">{message}</div>
}
